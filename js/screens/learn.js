...
function learnHTML() {
  ...
  const quizAnswered = state.quizAnswers[q.id] !== undefined;
    ...
    <div class="screen" style="height: 100%; display: flex; flex-direction: column;">
      ...
      <section class="learn-stack" style="flex: ${state.mythOpened ? '1' : 'none'}; display: flex; flex-direction: column; margin-bottom: 16px;">
        <div class="learn-card-head" data-action="toggle-myth" style="${state.mythOpened ? 'cursor: pointer; padding: 14px; background: var(--card); border: 1px solid var(--line); border-radius: 16px; margin-bottom: 16px;' : 'cursor: pointer; padding: 24px; display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 14px; background: var(--card); border: 1px solid var(--line); border-radius: 24px; box-shadow: 0 4px 20px rgba(0,0,0,0.15);'}">
          <span style="${state.mythOpened ? 'display: flex; align-items: center; gap: 8px;' : 'font-size: 20px; display: flex; align-items: center; gap: 10px;'}"><i data-lucide="shield-question" style="${state.mythOpened ? 'width: 20px; height: 20px;' : 'width: 32px; height: 32px;'}"></i> Myths vs facts</span>
          <b style="${state.mythOpened ? 'font-size: 14px;' : 'font-size: 15px;'}">${mythAnsweredCount >= 5 ? 'All 5 completed' : (state.mythOpened ? 'Close' : 'Tap to open')}</b>
        </div>
      ...
      <section class="learn-stack" style="flex: ${state.quizOpened ? '1' : 'none'}; display: flex; flex-direction: column;">
        <div class="learn-card-head quiz" data-action="toggle-quiz" style="${state.quizOpened ? 'cursor: pointer; padding: 14px; background: var(--card); border: 1px solid var(--line); border-radius: 16px; margin-bottom: 16px;' : 'cursor: pointer; padding: 24px; display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 14px; background: var(--card); border: 1px solid var(--line); border-radius: 24px; box-shadow: 0 4px 20px rgba(0,0,0,0.15);'}">
          <span style="${state.quizOpened ? 'display: flex; align-items: center; gap: 8px;' : 'font-size: 20px; display: flex; align-items: center; gap: 10px;'}"><i data-lucide="timer" style="${state.quizOpened ? 'width: 20px; height: 20px;' : 'width: 32px; height: 32px;'}"></i> Daily quiz</span>
          <b style="${state.quizOpened ? 'font-size: 14px;' : 'font-size: 15px;'}">${quizAnswered ? 'Explained' : (state.quizOpened ? 'Close' : 'Tap to open')}</b>
        </div>
  ...
  if (state.mythFlowIndex >= 5) {
    return `
      <div class="card" style="flex: 1; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; border-radius: 24px; box-shadow: 0 8px 30px rgba(0,0,0,0.12); padding: 40px 24px;">
        ...
        <h2 style="font-size: 1.5rem; margin-bottom: 12px;">All 5 Cleared!</h2>
        <p style="color: var(--text-muted); margin-bottom: 32px;">You've uncovered the facts. Great job!</p>
        <button class="btn btn-primary" data-action="close-myth-flow" style="width: 100%; padding: 16px; border-radius: 16px; font-size: 1.1rem;">Back to Learn</button>
      </div>
    `;
  }
...
function dailyQuizCardHTML(q) {
  ...
  return `
    <div class="card quiz-card" style="box-shadow: 0 8px 30px rgba(0,0,0,0.12); border-radius: 24px; padding: 24px;">
      <span class="eyebrow" style="margin-bottom: 16px;"><i data-lucide="timer"></i> Daily quiz</span>
      <div class="card-title" style="font-size: 1.2rem; line-height: 1.5; margin-bottom: 24px;">${q.question}</div>
      <div class="quiz-options" style="display: flex; flex-direction: column; gap: 12px;">
        ${q.options.map((opt, i) => `
          <button class="quiz-option ${state.quizAnswers[q.id] === i ? 'picked' : ''}" data-action="answer-quiz" data-id="${q.id}" data-index="${i}" style="text-align: left; padding: 16px; border-radius: 16px; border: 2px solid ${state.quizAnswers[q.id] === i ? 'var(--primary)' : 'var(--line)'}; font-size: 1.05rem;">
